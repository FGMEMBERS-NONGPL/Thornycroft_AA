...
var loop = func {
	...
	if (gear_prev != gear.getValue()) {
	 ...
	 k = 0;
	 if (running.getValue() == 1) {rpm.setValue(300+1700*elevator.getValue())} else {rpm.setValue(0)};
      }
	...
	if ((running.getValue() == 1) and (gear.getValue() > 0)) { 
	 throttle.setValue(1.25*elevator.getValue()+0.1-rpm.getValue()/1800);
	...
	if ((cranking.getValue() == 1) and ((gear.getValue() == 0) or (clutch.getValue == 1)) and (fuel.getValue()>=0.00000139)) {
	 running.setValue(1);	 
	 cranking.setValue(0);
	}
	...
	if (running.getValue() == 1) {
	 if (fuel.getValue() < 0.00000139) {
	  running.setValue(0);
	  }
	 else {
	  fuel_lev = fuel.getValue();
	  fuel.setValue(fuel_lev - (0.9*elevator.getValue()+0.1)*0.00000139);
	 }
	}

	if (running.getValue() == 1) {
	 if (gear.getValue() == -1) {
	  rpm.setValue(350*speed);
	  rev_throttle.setValue(0.9*elevator.getValue()+0.1-rpm.getValue()/1800);
	...
	mbrake = rpm.getValue()/1800-(1.0*elevator.getValue()+0.25*running.getValue());
	...
	if ((gear.getValue() != 0) and (getprop("/gear/gear/rollspeed-ms") != 0) and (clutch.getValue() == 0) and (tbrake < mbrake)) {setprop("/engines/engine/rev_throttle", mbrake)}
  	...
  	if (gear.getValue() == 1) {
	 setprop("/controls/gearstick-x", -4);
	 setprop("/controls/gearstick-y", 8);
	}
      if (gear.getValue() == 2) {
	 setprop("/controls/gearstick-x", -4);
	 setprop("/controls/gearstick-y", -8);
	...
	if (gear.getValue() == -1) {
	 setprop("/controls/gearstick-x", -4);
	 setprop("/controls/gearstick-y", 14);
	}
	if (gear.getValue() == 0) {
...
loop();
aircraft.livery.init("Aircraft/Thornycroft_AA/Models/Liveries", "sim/model/livery/name");
